...
class ChallengeAdapter(
    ...
    private val onTotalChanged: (challenge: Challenge) -> Unit
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int) =
        ChallengeViewHolder(LayoutInflater.from(parent.context).inflate(R.layout.item_challenge, parent, false))
    ...
    override fun onBindViewHolder(holder: ChallengeViewHolder, position: Int) {
        ...
        val daysLeft   = ((endCal.timeInMillis - today.timeInMillis) / 86_400_000L).toInt()
        val isComplete = challenge.amountSaved >= challenge.budgetMax
        val isExpired  = daysLeft < 0
        val (statusText, statusColor) = when { isComplete -> "Completed" to Color.parseColor("#2E7D32"); isExpired -> "Expired" to Color.parseColor("#B71C1C"); else -> "Active" to Color.parseColor("#1565C0") }
        val progressPct = if (challenge.budgetMax > 0) ((challenge.amountSaved.toFloat() / challenge.budgetMax) * 100).toInt().coerceIn(0, 100) else 0
        ...
        holder.txtDaysLeft.text    = when { isComplete -> "Challenge completed!"; daysLeft == 0 -> "Last day today!"; daysLeft > 0 -> "$daysLeft day${if (daysLeft == 1) "" else "s"} remaining"; else -> "Ended ${-daysLeft} day${if (-daysLeft == 1) "" else "s"} ago" }
        ...
        val addVis = if (isComplete) View.GONE else View.VISIBLE
        holder.editAddAmount.visibility = addVis; holder.btnAddMoney.visibility = addVis
        ...
        holder.btnAddMoney.setOnClickListener {
            ...
            CoroutineScope(Dispatchers.IO).launch {
                ...
                withContext(Dispatchers.Main) {
                    challenges[pos] = updated; holder.editAddAmount.setText(""); holder.editAddAmount.clearFocus()
                    (holder.itemView.context.getSystemService(android.content.Context.INPUT_METHOD_SERVICE) as android.view.inputmethod.InputMethodManager).hideSoftInputFromWindow(holder.editAddAmount.windowToken, 0)
                    notifyItemChanged(pos); onTotalChanged(updated)
                }
            }
        }

        holder.btnEdit.setOnClickListener { val pos = holder.adapterPosition; if (pos != RecyclerView.NO_POSITION) onEditClicked(challenges[pos], pos) }
            ...
            AlertDialog.Builder(holder.itemView.context)
                ...
                .setPositiveButton("Delete") { dialog, _ ->
                    CoroutineScope(Dispatchers.IO).launch {
                        ...
                        try { FirebaseRepository.deleteChallenge(challenge) } catch (e: Exception) { e.printStackTrace() }
                        withContext(Dispatchers.Main) { if (pos < challenges.size) { challenges.removeAt(pos); notifyItemRemoved(pos); notifyItemRangeChanged(pos, challenges.size); onTotalChanged(challenge) } }
                    }
                    dialog.dismiss()
                }
